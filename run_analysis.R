...
x.train <- read.table(paste0(trainPath, "/x_train.txt"))
message("Loading TRAINING data Y...")
y.train <- read.table(paste0(trainPath, "/y_train.txt"))
message("Loading TRAINING data SUBJECT...")
subject.train <- read.table(paste0(trainPath, "/subject_train.txt"))
...
x.test <- read.table(paste0(testPath, "/x_test.txt"))
message("Loading TEST data Y...")
y.test <- read.table(paste0(testPath, "/y_test.txt"))
message("Loading TEST data SUBJECT...")
subject.test <- read.table(paste0(testPath, "/subject_test.txt"))
...
write.table(summaryByActivitySubject, paste0(analysisPath, "/HAR Dataset - Summarized by Activity and Subject.txt"), row.names = FALSE)
